...
const config: Record<
  SpecializedType,
  { icon: typeof Car; href: string; type: SpecializedType }
> = {
  accident: { icon: Car, href: destinations.accident_wakeely, type: "accident" },
  tenant: { icon: HomeIcon, href: destinations.tenant_wakeely, type: "tenant" },
  labor: { icon: Briefcase, href: destinations.labor_wakeely, type: "labor" },
};
    ...
    <section className="border-t border-border bg-surface-alt" aria-labelledby="specialized-heading">
      <div className="container-page py-16">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          {cards.map((key) => {
            const { icon: Icon, href } = config[key];
            return (
              <a
                key={key}
                href={href}
                ...
                className="group flex flex-col gap-3 rounded-2xl border border-border bg-surface p-6 transition-all hover:border-brand-teal hover:shadow-lg hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus focus-visible:ring-offset-2"
              >
